...
namespace GoogleSheetsForUnity
{
    ...
    */
    public class SpreadsheetsController : MonoBehaviour
    {
        ...
        private PlayerInfo _playerData = new PlayerInfo { clinic = "魔法アプリ", patient = "001", time = "10:23", sads = "苦手度:7 メモ：しんどい" };

        // For the table to be created and queried.
        private string _tableName = "患者シート";
        ...
        private void OnGUI()
        {
            ...
            _playerData.clinic = GUILayout.TextField(_playerData.clinic, GUILayout.MaxWidth(100f));
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            GUILayout.Space(10f);
            GUILayout.Label("Player Level:", GUILayout.MaxWidth(100f));
            _playerData.patient = GUILayout.TextField(_playerData.patient.ToString(), GUILayout.MaxWidth(100f));
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            GUILayout.Space(10f);
            GUILayout.Label("Player Health:", GUILayout.MaxWidth(100f));
            _playerData.time = GUILayout.TextField(_playerData.time.ToString(), GUILayout.MaxWidth(100f));
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            GUILayout.Space(10f);
            GUILayout.Label("Player Role:", GUILayout.MaxWidth(100f));
            _playerData.sads = GUILayout.TextField(_playerData.sads, GUILayout.MaxWidth(100f));
            GUILayout.EndHorizontal();
            GUILayout.Space(5f);
                ...
                if (string.Compare(dataContainer.objType, _tableName) == 0)
                {
                    ...
                    for (int i = 0; i < players.Length; i++)
                    {
                        ...
                        Debug.Log("<color=yellow>Object retrieved from the cloud and parsed: \n</color>" +
                            "Name: " + _playerData.clinic + "\n" +
                            "Level: " + _playerData.patient + "\n" +
                            "Health: " + _playerData.time + "\n" +
                            "Role: " + _playerData.sads + "\n");
                    }
                }
            }
